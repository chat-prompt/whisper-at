...
# Modified to extract only the new SONYC classes that are not in AudioSet
...
def parse_args():
    parser = argparse.ArgumentParser(description='Process SONYC-UST annotations CSV for Whisper-AT training (new classes only)')
...
def process_annotations(annotations_df, class_mapping, audioset_classes, verified_only=False):
    ...
    Only extracts files containing new SONYC classes not mapped to AudioSet
    """
    ...
    fine_class_to_col = {}
    # Keep track of which classes are new (not in AudioSet)
    new_sonyc_classes = set()
    ...
    for col in presence_cols:
        if '-' in col:
            ...
            if len(parts) == 2 and parts[1] != 'X':  # Skip uncertain classes
                ...
                # Check if this is a new class (not mapped to AudioSet)
                if fine_name in class_mapping:
                    if class_mapping[fine_name] is None:
                        new_sonyc_classes.add(fine_name)
                else:
                    new_sonyc_classes.add(fine_name)
    
    print(f"Identified {len(new_sonyc_classes)} new SONYC classes not in AudioSet:")
    for cls in sorted(new_sonyc_classes):
        print(f"  - {cls}")
    
    # Create a mapping from audio filenames to labels (only for files with new classes)
    ...
    # Keep track of how many files have new classes
    files_with_new_classes = 0
    
    for audio_filename, group in grouped:
        # Check if this file has any new SONYC classes
        has_new_class = False
        file_labels = set()
        
        # Process each presence column to find new classes
        for fine_name, col in fine_class_to_col.items():
            # Check if this fine class is present in this file
            if (group[col] == 1).any():
                # If it's a new class (not mapped to AudioSet), mark this file
                if fine_name in new_sonyc_classes:
                    has_new_class = True
                    file_labels.add(f"sonyc_{fine_name}")
                # If the class has a mapping to AudioSet, add it to file_labels
                elif fine_name in class_mapping and class_mapping[fine_name] is not None:
                    audioset_class = class_mapping[fine_name]
                    if audioset_class in audioset_classes:
                        file_labels.add(audioset_classes[audioset_class])
        
        # Only keep files that have at least one new class
        if has_new_class:
            files_with_new_classes += 1
            # Record the split for this file
            split = group['split'].iloc[0]
            audio_to_split[audio_filename] = split
            # Store all labels for this file
            audio_to_labels[audio_filename] = file_labels
    
    print(f"Found {files_with_new_classes} files containing new SONYC classes")
...
def create_whisper_at_data(audio_to_labels, audio_to_split, audio_dir, new_class_mapping, index_to_mid, output_dir):
    ...
    with open(os.path.join(output_dir, "sonyc_new_train.json"), 'w') as f:
        json.dump(train_json, f, indent=2)
    
    with open(os.path.join(output_dir, "sonyc_new_val.json"), 'w') as f:
        json.dump(val_json, f, indent=2)
    
    with open(os.path.join(output_dir, "sonyc_new_test.json"), 'w') as f:
        json.dump(test_json, f, indent=2)
    
    print(f"Created data files with {len(train_data)} training, {len(val_data)} validation, and {len(test_data)} test examples")
    
    # Let's also print class distribution
    ...
    # Get class names for new SONYC classes
    class_names = {}
    for class_name, index in new_class_mapping.items():
        display_name = class_name.replace('sonyc_', '')
        mid = f"/m/sonyc_{display_name}"
        class_names[mid] = display_name
    
    # Print counts for new classes
    for mid, count in sorted(class_counts.items(), key=lambda x: x[1], reverse=True):
        if 'sonyc' in mid:
            print(f"  - {class_names.get(mid, mid)}: {count} instances")
...
def generate_sample_weights(train_json, audioset_csv, new_class_mapping, output_dir):
    ...
    Modified to give higher weights to new SONYC classes
    """
    ...
    # Calculate weights (1000 / count) with extra weight for new classes
    label_weight = 1000.0 / (label_count + 0.01)
    # Give 3x weight to new SONYC classes
    for class_name, index in new_class_mapping.items():
        label_weight[index] *= 3.0
    ...
    # Save weights
    weight_path = os.path.join(output_dir, "sonyc_new_train_weight.csv")
...
def main():
    ...
    # Process the annotations - only get files with new SONYC classes
    ...
    print(f"  - Training data: {os.path.join(args.output_dir, 'sonyc_new_train.json')}")
    print(f"  - Validation data: {os.path.join(args.output_dir, 'sonyc_new_val.json')}")
    print(f"  - Test data: {os.path.join(args.output_dir, 'sonyc_new_test.json')}")
